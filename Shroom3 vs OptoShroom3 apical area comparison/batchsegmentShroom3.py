...
dirpath = r'\\ebisuya.embl.es\ebisuya\Guillermo\Optoshroom3 paper\Python scripts and raw data\Shroom3VolumeMeasurements\Shroom3Files'
...
for q in range(len(filepaths)):
    ...
    datapath =  join(r'\\ebisuya.embl.es\ebisuya\Guillermo\Optoshroom3 paper\Python scripts and raw data\Shroom3VolumeMeasurements\ShroomData', originalname)
    segpath =  join(r'\\ebisuya.embl.es\ebisuya\Guillermo\Optoshroom3 paper\Python scripts and raw data\Shroom3VolumeMeasurements\Shroom3Segments', originalname)
    ...
    #Here we will need to pick which cells are expressing Shroom3, so their measurements are saved
    plt.figure()
    plt.subplot(1,2,1)
    plt.imshow(apicalcuts[1,:,:])
    for i in np.unique(apicalcuts[1,:,:]):
        if i == 0:
            continue
        else:
            tempcell = apicalcuts[1,:,:] ==i
            tempcell = tempcell.astype(int)
            Props = measure.regionprops(tempcell)
            y, x =Props[0].centroid
            plt.text(x-10,y+10, str(int(i)))
    plt.subplot(1,2,2)
    plt.imshow(image[-10,1,:,:])
    plt.show()
    ...
    while quest != "n":
        if quest != 0:
            chosen.append(int(quest))
        quest =input("Choose cells, if done write 'n'")

    print(chosen)
    print(len(listofcells))
    print(len(areas))
    print(len(basalareas))
    print(len(unilengths))
    chosenvols = []
    chosenaps = []
    chosenbas = []
    chosenlens = []
