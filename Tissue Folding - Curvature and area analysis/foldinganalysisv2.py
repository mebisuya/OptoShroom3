# -*- coding: utf-8 -*-
"""
Created on Thu Nov 19 14:46:19 2020

@author: ara
"""

###New file for analysis of curvature and axis reduction in 30X images. 
...
path = r'C:\Users\ara\Documents\Python Scripts\OptoShroom Folding\Cured Samples'
...
#To run 3D segmentation because I don't know how to measure 3D curvature.
...
def update(tp):  
    ...
    plt.imshow(rotProj[tp,:,:], interpolation='none')
    plt.imshow(rotMask[tp,:,:], interpolation='none', alpha=0.2)
    plt.imshow(MainMask, interpolation='none', cmap='spring')
    plt.imshow(MinorMask, interpolation='none', cmap='spring')
    
ani = FuncAnimation(fig, update, frames = time)  

writer = PillowWriter(fps=10)  
ani.save(filepath[:-4] + "projections.gif", writer=writer) 
...
def update(tp):  
    ...
    plt.imshow(Maincrop[tp,:,:], interpolation='none')
    plt.imshow(resmainmask[tp,:,:], interpolation='none', alpha = 0.2)
    plt.imshow(SkelMask, interpolation='none', cmap='spring')
    
ani = FuncAnimation(fig, update, frames = time)  

writer = PillowWriter(fps=10)  
ani.save(filepath[:-4] + "mainaxis.gif", writer=writer)  
...
def update(tp):  
    ...
    SkelMask = ndi.binary_dilation(SkelMask) #Dilation to make it more visible when ploting!
    SkelMask = np.ma.masked_where(SkelMask<1, SkelMask)
    plt.imshow(Minorcrop[tp,:,:], interpolation='none')
...
writer = PillowWriter(fps=10)  
ani.save(filepath[:-4] + "minoraxis.gif", writer=writer)  
...
#plt.show()
plt.savefig(filepath[:-4] + 'AreaVariation.png', bbox_inches='tight')
...
plt.legend()
#plt.show()
plt.savefig(filepath[:-4] + 'AxSizeVariation.png', bbox_inches='tight')
...
plt.legend()
#plt.show()
plt.savefig(filepath[:-4] + 'MeanCurvatures.png', bbox_inches='tight')
...
if allgood == "y":
    ...
    DataOutputFolder = r"C:\Users\ara\Documents\Python Scripts\OptoShroom Folding\Cured Data"
