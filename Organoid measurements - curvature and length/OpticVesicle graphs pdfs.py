...
#plt.show()
plt.savefig(FolderToSave + '/' +'Vesiclediametersfig.pdf', bbox_inches='tight', transparent=True)
...
fig, ax = plt.subplots(figsize = (0.7, 2.4))
#plt.title('Lumen diameter in stimulated samples')
ax.scatter([1 for i in initialsizest], initialsizest, c = '#4f81c2', alpha =0.7, s=7)
#plt.scatter([1 for i in initialsizes[4]], initialsizes[4], c = 'purple', label = 'day11')
ax.scatter([2 for i in endsizest], endsizest, c = '#dc756d', alpha =0.7, s=7)
#plt.scatter([2 for i in endsizes[4]], endsizes[4], c = 'purple', label = 'day11')
for i in range(len(initialsizest)):
    ax.plot([1,2],[initialsizest[i], endsizest[i]], c='black', alpha =0.2)
...
fig.savefig(FolderToSave + '/' + 'EyecupDiameterChangeFig.pdf', bbox_inches='tight', transparent=True)
plt.show()
