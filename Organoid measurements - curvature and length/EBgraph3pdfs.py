...
time = np.amin([len(listoffiles[i][0]) for i in range(len(listofnames))])
print(time)
...
for t in range(time):
    ...
    for n in range(NumbColonies):
        temp.append(listoffiles[n][0][t]/listoffiles[n][0][0]) #Relative area
        if t ==0:
            initialareas.append(listoffiles[n][0][t])
    mean = np.mean(temp)
    sd = np.std(temp)
    area.append(mean)
    areaplussd.append(mean+sd)
    areaminussd.append(mean-sd)
...
ax.plot([-180, 180], [meanexpectedcurvature, meanexpectedcurvature], color= 'k', label = 'Expected curvature', alpha = 0.4, linewidth=1)
ax.plot(ave[t1,:,0],ave[t1,:,1], color = "#4f81c2", alpha = 0.8, label = "Before stimulation", linewidth=1)
ax.fill_between(ave[t1,:,0],sdup[t1,:,1],sdlo[t1,:,1], color='#4f81c2', alpha=.1)
ax.plot(ave[t2,:,0],ave[t2,:,1], color = "#dc756d", alpha = 0.8, label = "1 h stimulation", linewidth=1)
ax.fill_between(ave[t2,:,0],sdup[t2,:,1],sdlo[t2,:,1], color='#dc756d', alpha=.1)
ax.set_ylabel("Curvature " +  r'$ (μm^{-1})$')
ax.set_xlabel("Orientation (degrees)")
ax.axvspan(lowerstimangle, uppersimangle, color = '#a4c3f6', alpha = 0.3, fill = "True", lw= 0)
ax.legend(loc = "lower left", fontsize = 6)
ax.spines['right'].set_visible(False)
ax.spines['top'].set_visible(False)
fig.savefig(FolderToSave + "/" + d1 + 'maincurvaturesfig.pdf', bbox_inches='tight', transparent=True)
plt.show()
